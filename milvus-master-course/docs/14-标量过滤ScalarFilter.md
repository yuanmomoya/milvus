...
标量过滤与向量搜索结合时，Milvus 提供标准过滤和迭代过滤两种思路：
...
flowchart TB
    ...
    B --> C["标准过滤"]
    C --> C1["先计算标量条件位图"]
    C1 --> C2["在满足条件的数据范围内做 ANN"]
    B --> D["迭代过滤"]
    D --> D1["分批获取向量候选"]
    D1 --> D2["逐个检查复杂标量条件"]
```

- **标准过滤**适合大多数表达式，尤其是字段已有标量索引时。
- **迭代过滤**适合复杂 JSON、字符串或算术表达式导致标量计算成本过高的场景，需要通过 search 参数显式选择。
- 性能不仅由过滤比例决定，还取决于表达式复杂度、标量索引、候选规模和向量索引类型。
...
| JSON 字段过滤慢 | 常用 JSON 路径没有索引 | 建立 JSON 路径索引，或把稳定高频字段显式定义 |
...
5. **动态字段能建索引吗？**
   可以为动态字段中的常用键建立 JSON 路径索引。核心过滤字段仍建议显式定义，以获得清晰的类型约束和维护边界。
